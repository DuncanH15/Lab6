...
public class Sudoku extends LatinSquare implements Serializable {
	...
	protected static HashMap<Integer, SudokuCell> cells = new HashMap<Integer, SudokuCell>();
	...
	public int getMistakes(int iRow, int iCol, int iValue, int iDifficulty) {
		if(isValidValue(iRow, iCol, iValue) != true) {
			mistakeCount++;
		} return mistakeCount;
	}
	...
	public void GameOver() {
		if(isgameOver(mistakeCount, eGameDifficulty.getiDifficulty()) == true) {
	...
	private class SudokuCell extends Cell {
		...
		public int zeroCount() {
			int zeroCounter = 0;
			int i;
			for(GetNextCell.getCellValue() = 0; i < iSize; GetNextCell()) {
				zeroCounter++;
			}
			}
		}
