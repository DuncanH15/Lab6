				...
				paneTarget.setOnDragDropped(new EventHandler<DragEvent>() {
					public void handle(DragEvent event) {
						...
						if (db.hasContent(myFormat)) {
							...
							if (!s.isValidValue(CellTo.getiRow(), CellTo.getiCol(), CellFrom.getiCellValue())) {
								if (game.getShowHints()) {
									if(s.isgameOver(s.getMistakes(CellTo.getiRow(), CellTo.getiCol(), CellTo.getiCellValue(), CellTo.getiDifficulty()), CellTo.getiDifficulty()) == true) {
										System.out.println("Game Over, press 'Start Game' to try again");
									}
